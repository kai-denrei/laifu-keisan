...
| 2026-05-26 | v9: new module `js/pwa-install.js` added to SW `ASSETS_BARE` precache (bare-import only — never referenced with `?v=`). | Bare-imported modules must have their bare form precached for offline launch. | [[dev]] |
| 2026-05-26 | **GitHub identity convention: all git work on kainode uses the Kai Denrei account/persona, never Gerald's (Jelaludo's) personal GitHub.** Git identity set GLOBALLY: `user.name="Kai Denrei"`, `user.email="270854086+kai-denrei@users.noreply.github.com"` (GitHub ID-based noreply — attributes to the account without writing the Apple-relay email into public history). Documented in `~/CLAUDE.md` + vault `nodes/github.md`. | `gh` was already authed as kai-denrei (push/PR routing was correct), but commit AUTHOR was unset → inherited a stale per-repo `user.email` (gfabrot@gmail.com) and authored commit d3ccd37 as Jelaludo. Global default + a per-repo-override check prevent recurrence. | [[pm]] |
...
| 2026-05-26 | Deployed a source change (radial history fix, commit d3ccd37) by editing `layout.css` + pushing — WITHOUT running `scripts/bust.sh`. | Token unchanged → `sw.js` unchanged → installed PWAs neither updated the SW nor re-fetched the cache-first `layout.css?v=<old>`. The fix reached new visitors but NOT existing installs, with no update toast. Corrected in v9: bumping the token (9fbe74cd → bd585b5d) now delivers both the history fix and the PWA work. |
| 2026-05-26 | Committed with the repo's inherited LOCAL git identity (no global default was set on kainode). | Local `user.email` resolved to `gfabrot@gmail.com` → commit d3ccd37 authored as "Gerald (Jelaludo)" on the kai-denrei repo. Wrong persona. Fixed v9: global identity set to Kai Denrei + documented the per-repo-override trap. |
...
- Editing source without bumping the cache-bust token is a no-op for installed PWA users: the SW only updates when `sw.js` changes (token-keyed), and `?v=`-fingerprinted assets are cache-first, so the old URL keeps serving old bytes. Every deploy runs `scripts/bust.sh` — "it works for new visitors" hides this completely. — from dead end on 2026-05-26
- On a shared agent machine, a repo's LOCAL `git config user.email` silently overrides the global identity. Set the intended identity globally AND check `git config user.email` before the first commit in any repo — an inherited override authors commits under the wrong account with no warning. — from dead end on 2026-05-26

## Open Questions
- [x] ~~Hosting target~~ — Resolved in v1: GitHub Pages.
- [ ] **CI/CD hook for bump** — `bust.sh` runs on dev save (via `watch.sh`). On deploy: currently manual (run `./scripts/bust.sh` before each commit). Could wire as a pre-commit hook or a GitHub Actions workflow. — owner: Gerald — since: 2026-05-23 — **2026-05-26: the manual step was missed (d3ccd37 shipped unbumped, never reached installs). Raises priority — a pre-commit hook that bumps when `js/`,`css/`,`*.html`,`sw.js` are staged would have caught it.**
...
## Session Log
- 2026-05-26 — SYNC v9: precache pwa-install.js; recorded the missed-token-bump Dead End (d3ccd37 shipped unbumped → never reached installs) and the wrong-author Dead End; GitHub identity convention = Kai Denrei (global git identity + `~/CLAUDE.md` + vault `nodes/github.md`). 2 Decisions, 2 Dead Ends, 2 Lessons; bumped CI-hook OQ priority.
