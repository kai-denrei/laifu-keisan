...
# ---------- 4. Rewrite __CB_TOKEN__ / const CB_TOKEN in sw.js (PWA cache invalidation) ----------
# Per cache-busting skill references/service-worker.md: SW cache name keyed off the token
# so every bump atomically invalidates the SW's app-shell cache.
if [[ -f sw.js ]]; then
  if grep -qE 'const CB_TOKEN = "[^"]*"' sw.js; then
    sed "${SED_INPLACE[@]}" -E "s/const CB_TOKEN = \"[^\"]*\"/const CB_TOKEN = \"${TOKEN}\"/" sw.js
    rm -f sw.js.cbbak
    [[ -z "$QUIET" ]] && echo "  ✓ CB_TOKEN bumped in sw.js"
    REWRITTEN=$((REWRITTEN + 1))
  fi
fi
